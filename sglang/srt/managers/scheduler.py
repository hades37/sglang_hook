...
from sglang.srt.distributed.communication_hook import (
    is_hook_enabled,
    is_active_rank,
    install_communication_hooks,
)
logger = logging.getLogger(__name__)
_HOOK = os.getenv('SGLANG_HOOK', '0') == '1'
...
def run_scheduler_process(
    ...
    print(f"[DEBUG] run_scheduler_process started: tp={tp_rank}, dp={dp_rank}, pp={pp_rank}, gpu={gpu_id}", flush=True)

    # ========== HOOK 模式处理 ==========
    effective_dp_rank = dp_rank if dp_rank is not None else 0
    global_rank = (
        tp_rank
        + effective_dp_rank * server_args.tp_size
        + pp_rank * server_args.tp_size * server_args.dp_size
    )
    
    print(f"[DEBUG] global_rank={global_rank}, checking hook...", flush=True)
    
    _is_hook_enabled = is_hook_enabled()
    print(f"[DEBUG] is_hook_enabled={_is_hook_enabled}", flush=True)
    
    if _is_hook_enabled:
        _is_active = is_active_rank(tp_rank, effective_dp_rank, pp_rank)
        print(f"[DEBUG] is_active={_is_active}", flush=True)
        
        install_communication_hooks()
        print(f"[DEBUG] communication hooks installed", flush=True)
        
        if not _is_active:
            print(f"[DEBUG] Dummy rank {global_rank} sending ready...", flush=True)
            pipe_writer.send(
                {
                    "status": "ready",
            ...
            print(f"[DEBUG] Dummy rank {global_rank} entering sleep loop...", flush=True)
            while True:
                time.sleep(60)
            return
        
        print(f"[DEBUG] Active rank {global_rank} continuing...", flush=True)
    # ========== HOOK 模式处理结束 ==========

    print(f"[DEBUG] About to create Scheduler...", flush=True)
    
    # ... 后续代码 ...

    # 以下是原有代码，只有 active rank 才会执行到这里
