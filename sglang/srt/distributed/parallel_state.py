...
class GroupCoordinator:
    ...
    def __init__(
        ...
        if use_pynccl and self.world_size > 1:
            # HOOK: 检查是否启用了 hook 模式
            import os
            if os.environ.get("SGLANG_HOOK", "0") == "1":
                print(f"[HOOK] Skipping PyNcclCommunicator creation (world_size={self.world_size})", flush=True)
                # 创建一个假的 communicator 对象
                # 创建一个假的 communicator 对象
                from contextlib import contextmanager
                class _FakePyNcclComm:
                    def __init__(self):
                        self.available = True
                        self.disabled = True
                    def all_reduce(self, *args, **kwargs): return args[0] if args else None
                    def all_gather(self, *args, **kwargs): pass
                    def reduce_scatter(self, *args, **kwargs): pass
                ...
                self.pynccl_comm = _FakePyNcclComm()
            else:
        ...
        if use_pymscclpp and self.world_size > 1:
            import os
            if os.environ.get("SGLANG_HOOK", "0") == "1":
                print(f"[HOOK] Skipping PyMscclppCommunicator creation", flush=True)
        ...
        if use_custom_allreduce and self.world_size > 1:
            import os
            if os.environ.get("SGLANG_HOOK", "0") == "1":
                print(f"[HOOK] Skipping custom_allreduce creation", flush=True)
            else:
        ...
        if use_message_queue_broadcaster and self.world_size > 1:
            import os
            if os.environ.get("SGLANG_HOOK", "0") == "1":
                print(f"[HOOK] Skipping MessageQueue creation", flush=True)
